    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#ffffff] dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 bg-farm-blue text-[#ffffff] font-semibold text-sm rounded-xl shadow-md hover:bg-opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 shrink-0"
        >
      ...
      <div className="flex flex-wrap items-center gap-2 px-6 py-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 text-sm overflow-x-auto">
        <button
          onClick={() => setCurrentNodeId(undefined)}
          className={`flex items-center gap-1 font-semibold ${!currentNodeId ? 'text-farm-blue' : 'text-gray-500 dark:text-gray-400 hover:text-farm-blue'}`}
        >
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400 shrink-0">
          إجمالي العناصر المستعرضة: <span className="font-bold text-gray-800 dark:text-[#ffffff]">{filteredNodes.length}</span>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <span className="text-gray-500 dark:text-gray-400 text-sm mt-3 font-medium">جاري تحميل شجرة الأرشيف...</span>
        </div>
      ) : isError ? (
        <div className="flex items-center gap-3 p-5 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 rounded-2xl text-red-600 dark:text-red-400">
          ...
          {filteredNodes.map((node) => {
            ...
            return (
              <div
                ...
                className="group relative flex flex-col justify-between p-6 bg-[#ffffff] dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm hover:shadow-md hover:border-farm-blue dark:hover:border-farm-blue cursor-pointer transition-all duration-200"
              >
                <div>
                  <div className="flex items-center justify-between">
                    <div className={`p-3 rounded-xl ${
                      node.type === 'institution' ? 'bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400' :
                      node.type === 'year' ? 'bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400' :
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl transition-all duration-150"
                      title="حذف"
                    >
                  ...
                  <h3 className="text-base font-bold text-gray-800 dark:text-[#ffffff] font-sans mt-4 group-hover:text-farm-blue transition-colors">
                    {node.name}
                  </h3>
                ...
                <div className="flex items-center justify-between border-t border-gray-50 dark:border-gray-800 mt-6 pt-4 text-xs text-gray-500">
                  <span>
            ...
            <form onSubmit={handleCreateNode} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-farm-blue dark:text-[#ffffff]"
                />
              </div>

              {newNodeType !== 'year' && (
                <div>
                  <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">الوصف (اختياري)</label>
                  <textarea
                    value={newNodeDesc}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in" dir="rtl">
          <div className="bg-[#ffffff] dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl w-full max-w-md p-6 shadow-xl space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed font-sans space-y-2">
              <p>
                تنبيه: أنت على وشك حذف العنصر <span className="font-bold text-red-600 dark:text-red-400">"{deleteTarget.name}"</span> وكل ما يحتويه من مجلدات فرعية، مستندات، وملفات بشكل نهائي.
              </p>
              <p className="font-semibold text-gray-505 dark:text-gray-400 bg-gray-50 dark:bg-gray-850 p-2.5 rounded-lg border border-gray-100 dark:border-gray-800">
