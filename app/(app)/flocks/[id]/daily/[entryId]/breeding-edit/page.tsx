...
import { useBreedingEntry, useUpdateBreedingEntry } from '@/lib/hooks/useDailyOps'
...
export default function BreedingEntryEditPage() {
  ...
  const { data: entryData, isLoading: isEntryLoading, error: entryError } = useBreedingEntry(flockId, entryIdNum)
  const updateEntry = useUpdateBreedingEntry(flockId)
  ...
  useEffect(() => {
    if (entry) {
      ...
      setWeightAvg(entry.weight_sample_avg?.toString() ?? '')
      setUniformity(entry.uniformity_pct?.toString() ?? '')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: Parameters<typeof updateEntry.mutate>[0]['data'] = {
    ...
    <div className="max-w-3xl mx-auto space-y-6" dir="rtl">
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mt-2">
          <h1 className="text-2xl font-bold text-gray-900 font-outfit">تعديل القياس اليومي</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-6">
        {updateEntry.error && (
          <div className="p-4 bg-red-50 border-r-4 border-red-500 text-red-700 flex items-start gap-3 rounded-md">
            ...
            <div className="text-sm">
              <p>{(updateEntry.error as { message?: string })?.message ?? 'حدث خطأ أثناء تحديث القياس'}</p>
        ...
        {/* البيانات اليومية */}
        <div>
          <h3 className="text-sm font-bold text-gray-800 mb-3 border-b pb-2 border-gray-100">البيانات اليومية</h3>
          <div className="space-y-4">
            <div>
              <label htmlFor="mortality" className="text-sm font-semibold text-gray-700 block mb-1">النفوق</label>
              <input
                id="mortality"
                type="number"
                min="0"
            ...
            <div>
              <label htmlFor="weight_sample_avg" className="text-sm font-semibold text-gray-700 block mb-1">متوسط وزن العينة — جرام (اختياري)</label>
              <input
                id="weight_sample_avg"
                type="number"
                min="0"
                step="0.01"
                className={ic('weight_sample_avg')}
                value={weightAvg}
            ...
            <div>
              <label htmlFor="uniformity_pct" className="text-sm font-semibold text-gray-700 block mb-1">التجانس % (اختياري)</label>
              <input
                id="uniformity_pct"
                type="number"
                min="0"
