    ...
    <section className="rounded-2xl border border-slate-100 bg-white dark:bg-surface p-5 shadow-sm sm:p-6">
      ...
      <div className="grid grid-cols-1 gap-3 lg:hidden">
        {cycles.length === 0 ? (
          ...
          cycles.map((cycle) => (
            <article key={cycle.id} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:bg-slate-900/40">
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-extrabold text-slate-900">{cycle.name}</h3>
                  <p className="mt-1 truncate text-xs font-semibold text-slate-500">
                    {cycle.variety_name || 'غير محدد'}
                  </p>
                </div>
                <span className={`shrink-0 rounded-lg px-2 py-1 text-xs font-extrabold ${
                  cycle.status === 'active'
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-xl bg-white px-3 py-2 dark:bg-surface">
                  <p className="text-xs font-bold text-slate-500">الكمية القائمة</p>
                  <p className="mt-1 font-mono font-extrabold text-slate-900">{cycle.count.toLocaleString('ar-SA')}</p>
                </div>
                <div className="rounded-xl bg-white px-3 py-2 dark:bg-surface">
                  <p className="text-xs font-bold text-slate-500">طريقة الإكثار</p>
                  <p className="mt-1 font-bold text-slate-900">{cycle.propagation_type ? propagationLabels[cycle.propagation_type] : 'غير محدد'}</p>
                </div>
                <div className="rounded-xl bg-white px-3 py-2 dark:bg-surface">
                  <p className="text-xs font-bold text-slate-500">الموقع / الحوض</p>
                  <p className="mt-1 font-bold text-slate-900">{cycle.nursery?.name || ''} - {cycle.basin?.name || 'غير محدد'}</p>
                </div>
                <div className="rounded-xl bg-white px-3 py-2 dark:bg-surface">
                  <p className="text-xs font-bold text-slate-500">تاريخ البدء</p>
                  <p className="mt-1 font-mono font-bold text-slate-900">{cycle.start_date || 'غير محدد'}</p>
                </div>
              </div>
            </article>
          ))
